...
function doDelete(model: VehicleModel) {
  if (!confirm(`Are you sure you want to delete vehicle ${model.model.name}?`)) return
  VehicleService.deleteVehicle(model.vehicleId)
    .then(
      (rsp) => (vehicles.value = vehicles.value?.filter((v) => v.vehicleId !== model.vehicleId)),
    )
    .catch((e) => logout(e))
}
  ...
  <nav aria-label="breadcrumb" v-if="client">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">
        <span v-if="client.taxId"> {{ client.name }} ({{ client.taxId }}) </span>
